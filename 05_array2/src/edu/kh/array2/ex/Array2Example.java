...
//		arr[1][2] = 60;
		...
		System.out.println(arr[0].length); //0행 열길이
		//arr[0]이 참조하고 있는 배열이 길이
		
		System.out.println(arr[1].length); //1행 열길이
		//arr[1]이 참조하고 있는 배열이 길이
	...
	public void ex2() {
		...
		for (int col = 0; col < arr.length; col++) { // 열 반복(3)

			int sum = 0;

			for (int row = 0; row < arr[col].length; row++) { // 행 반복(3)
				sum += arr[row][col];
				// 0 0 : 1
				// 1 0 : 4
				// 2 0 : 7
	...
	public void ex3() {
		...
		arr[0] = new char[3]; //0행의 3열짜리 1차원 배열을 생성하여 주소값 저장
		arr[1] = new char[4]; //1행의 4열짜리 1차원 배열을 생성하여 주소값 저장
		arr[2] = new char[5]; //2행의 5열짜리 1차원 배열을 생성하여 주소값 저장
		arr[3] = new char[2]; //3행의 열짜리 1차원 배열을 생성하여 주소값 저장
