...
public class ByteService {
	...
	//IO 관련된 코드 IOException 발생될 가능성이 높다!
	...
	public void fileByteOutput() {
		...
		try {
			...
			//출력방법1 : 한글자(2byte)씩 파일로 출력
			
			//StringBuilder -> String 변환 -> chatAt() -> char 변환
			...
			*/
			...
			fos.write(content.getBytes()); //byte[]로 만들어 변환
			...
			e.printStackTrace();
		} finally { //try 예외 발생 여부 관게없이 무조건 수행
			
			//사용 완료한 스트림 제거(닫기, 메모리 반환)하는 코드 작성
			//->메모리 누수 방지
			...
			try {
				...
				if(fos!=null) fos.close(); //close에 flush 기능 포함되어있음
	 ...
	 */
	public void bufferedFfileByteOutput() {
		/*
		 * 버퍼(Buffer)란?
		 ...
		 * 
		 * - FileOutputStream은 1바이트씩 데이터 입출력
	...
	public void fileByteInput() {
		...
		try {
			...
			// -> 존재하지 않으면 FileVotFoundException 발생
	 ...
	 * -> BufferedReader 생성 시 보조스트림인 InputStreamReader -> InputStream(기반)
	 * 
	 */
