...
public class PolyService {
	...
	public void ex1() {
		...
		Car car3 = new Spark();
		// Spark 객체가 참조하는 변수의 타입이 Car
		// Spark 객체가 Car 객체로 변화함
		// ->업캐스팅
		
		//자식이 부모인척 Car인척 하는 것이기 때문에
		//Tesla나 Spark가 가진 속성은 사용할 수 없음
		...
		arr[1] = car2; // Car 인척하는 Tesla 주소
		
		arr[2] = car3; // Car 인척하는 Spark 주소
		
		for(int i=0; i<arr.length; i++) {
			System.out.println(i + "번째 인텍스의 엔진 : " + arr[i].getEngine());
		}
		
		//상속 + 다형성
		//상속 특징 : Car 상속 클래스는 모두 getEngine()을 가지고 있다.
		//다형성(업캐스팅) : 부모타입 참조변수 arr[i]로 자식객체를 참조할 수 있는 형태
		
	}

	//전달받은 Car 또는 자식 객체(Tesla, Spark)의 엔진, 연료, 바퀴갯수를 출력하는 메소드
	...
	public void ex2() {
		...
		System.out.println( arr[1] instanceof Spark); // false (스파크와 테슬라는 관계없음)
		System.out.println( arr[1] instanceof Car); // true (테슬라와 카는 상속관계)
	...
	public void ex3() {
		...
		//테슬라 객체의 getBatteryCapacity()를 사용할 수 있음
		
		Tesla t1 = (Tesla)c1; //Tesla t1 = new Tesla();
	...
	public void ex5() {
		...
		//edu.kh.poly.ex1.model.dto.Car.toString ()
		//참조변수 c2가 Car타입이므로
		//toString() 호출코드와 Car의 toString() 메소드 코드 연결
		//->정적 바인딩
		
		//실행해보면 자식(Spark)의 toString()이 호출되는 것을 확인
		// 경차엔진 / 휘발유 / 4 / 0.5
		// ->컴파일 시에는 부모(Car)와 바인딩 == 정적바인딩
		//-> "실행 시"에는 정적 바인딩보다 자식(Spark)에서 오버라이딩된
