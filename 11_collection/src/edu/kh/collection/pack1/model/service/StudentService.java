...
public class StudentService {
	...
	// java.util.ArrayList : 배열형태의 List(가장 대표적인 List 자식 클래스)
	...
	studentList.add(new Student("오미나", 37, "충북 청주시", 'M', 90));
	studentList.add(new Student("박주희", 24, "서울시 서대문구", 'M', 70));
	...
	public void ex() { //List 테스트용 에제
		...
		testList.add(sc);
		testList.add(new Object()); //본인 객체
		
		// 컬렉션의 특징 : 여러 타입의 데이터 저장 가능
		
		// Object List.get(index) : 리스트에서 index번째 인덱스에 있는 객체 반환
		// 반환형이 Object == 모든 객체 반환 가능
		 ...
		 //testList에 꺼내온 인덱스 요소가 Student 혹은 상속관게인지 검사
	...
	*/
	...
	public void displayMenu() {
		...
		do {
			...
			try {
				...
				// menuNum이 초기값이 0을 가지고 있어 반복문이 종료되는데,
	...
	public String addStudent() {
		...
		// Unchecked Exception : 강제 처리X, 예외 발생하면 자동으로 상위 호출 메서드로 예외 전파
	...
	public void selectAll() {
		...
		if(studentList.isEmpty()) {
			...
			return; //현재 메서드를 종료하고 호출한 곳으로 돌아감(맨몸으로 돌아감)
		}
		...
		*/
		...
		// - 컬렉션, 배열의 모든 요소를 순차적으로 반복 접근할 수 있는 for 문
	 ...
	 * 3. 학생 정보 수정 메서드
	 * 
	 * - 학생 정보가 없다면 studentList에 하나라도 있는지 검사,
	...
	public void searchName1() {
		...
		for(Student std : studentList) {
			//타입 선언 : 리스트이름
			if(input.equals(std.getName())) { //이름 일치
				System.out.println(std); //std.toSting;
	 ...
	 * 
	 * 문자열 입력받아 studentList에서 꺼내온 StudentList에서 꺼내온
	...
	public void sortByAge() { 
		...
		Collections.sort(studentList); //Student class 가서 compareTo(Student other) 보기
	...
	public void sortByName() {
		...
		Collections.sort(studentList, new Comparator<Student>() {
			...
			public int compare(Student o1, Student o2) {
				...
				//return o2.getName().compareTo(o1.getName()); //거꾸로순
